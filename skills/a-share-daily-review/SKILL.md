...
- External background: optional `external_background.v1` or `external_background_fusion.v1` JSON from `daily-financial-briefing`; it may be merged into market structure, risk observations, and follow-up questions, but must not create a standalone HTML section, enter local market data sources, or complete missing A-share evidence.
