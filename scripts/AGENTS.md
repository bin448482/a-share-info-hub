...
- `run_daily_report_job.py`：每日定时采集和报告发送的薄编排入口；按阶段调用 `.venv/bin/python -m a_share_info_hub daily-update`、`daily-review` 和 Claude Code 非交互命令，默认生成 `external-background-fusion.json` 后再生成 `llm-review-sections.json`，并写入 `reports/daily-jobs/YYYY-MM-DD/` 状态、heartbeat、摘要和可选 OpenClaw `feishu` channel HTML 附件发送记录，不直接实现采集或 HTML 渲染。
