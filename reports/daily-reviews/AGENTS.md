...
- `YYYY-MM-DD/external-background-fusion.json`：外部背景流程汇总出的 `external_background_fusion.v1`，由 `$a-share-daily-review` skill 的 6 并行子 Agent 生成，可传给 `daily-review --external-background`。
...
- `2026-06-18/external-background-passed-simulated.json`：第一阶段用于验证 external background 融入主报告的 legacy 模拟输入；不再作为外部背景验收样例。
...
- `2026-06-18/a-share-daily-review-data-notes.md`：基于当前 `2026-06-18` daily run 生成的技术参考文件，记录 `partial` 状态、板块接口失败细节。
