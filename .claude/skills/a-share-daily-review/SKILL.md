...
7. If external background is requested and no ready `external_background.v1` or `external_background_fusion.v1` JSON is provided:
   ...
   - Spawn 6 sub-agents in parallel. Each sub-agent handles one topic, must use `$daily-financial-briefing`, and must stay inside the local evidence boundary. Use the sub-agent prompt template in [daily-review-workflow.md](references/daily-review-workflow.md) to construct each sub-agent prompt.
