...
- `vars.external_background_state`：可选，模拟 `external_background.v1` 输入状态。
...
- HTML 输出包含 `1.1 大盘`，并分出 `大盘定性` 和 `大盘结构`。
- external background passed 时，HTML 展示 `外部宏观与机构观点背景`、风险观察、待验证问题和参考来源。
- external background blocked 时，本地 HTML 继续生成，但不展示外部结论正文。
- external background invalid citation 时，无 URL 核心点不进入 HTML，技术参考 Markdown 记录降级原因。
...
- context 是否生成。
- HTML 是否生成。
- HTML 是否包含 `1.1 大盘 / 大盘定性 / 大盘结构`。
- 技术参考 Markdown 是否生成。
- external background 是否只进入独立 context 字段、HTML 背景章节和技术参考 Markdown。
...
- `contains`：检查 `1.1 大盘`、`大盘定性`、`大盘结构`。
- `contains`：检查 `外部宏观与机构观点背景`、`external_background_diagnostics_present: true`、引用 URL 和“仍需用 A 股行情、板块和情绪数据验证”。
