...
- external background passed 时，HTML 不新增独立外部背景章节，外部变量融合进 `风险观察` 和 `下一步研究问题`，引用写入技术参考 Markdown。
...
- `contains`：检查 `external_background_diagnostics_present: true`、`external_background_standalone_sections: 0`、`risk_section_count: 1`、`follow_up_section_count: 1` 和“仍需用 A 股行情、板块和情绪数据验证”。
