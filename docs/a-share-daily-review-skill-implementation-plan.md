...
```json
{
  ...
  "data_status": "passed|partial|skipped|failed|missing",
  "data_sources_used": [],
  "blocked_sections": [],
  "source_health": {},
  "market_breadth": {},
...
- `skipped`、`failed` 或 `missing` 状态下，context 只能允许数据边界说明、阻断原因和修复建议。
...
- `partial`、`skipped`、`failed`、`missing` 状态必须在 HTML 顶部显著展示。
...
- `skipped`：目标日期不是 A 股交易日，每日采集已跳过，不能生成市场复盘结论。
- `failed`：主表失败、主表为空、主表标准化不可读、DuckDB 必需查询失败，或状态报告显示当日失败。
- `missing`：找不到指定交易日 daily run，或缺少可判断状态的关键文件。

状态降级规则：

- `skipped`、`failed` 或 `missing` 时，不生成市场复盘结论，只输出阻断原因和下一步修复建议。
...
- 读取到 `overall_status=skipped` 时，状态保持为 `skipped`，不因缺少主表降级为 `failed`。
...
6. `skipped` 状态说明非交易日跳过
   - 输入：`interface-status.json` 中 `overall_status=skipped` 且 `sources=[]`。
   - 断言：输出 `skipped`，生成 context，但不生成市场复盘结论，也不提示重跑同一天采集。

7. `missing` 状态不读取其他日期
   - 输入：指定日期没有 daily run，但其他日期存在。
   - 断言：输出 `missing`，提示公开 CLI 刷新方式，不读取其他日期。

8. DuckDB 降级到 Parquet
...
7. 测试满足
   - `passed`、`partial`、`skipped`、`failed`、`missing` 五类状态都有测试。
