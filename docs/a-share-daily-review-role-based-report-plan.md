...
> 后续修正：`docs/20260619-132232-daily-financial-briefing-html-integration-implementation-plan.md` 已把旧 `1.1 大盘` 标题调整为 `大盘观察`，并要求 external background 融入主报告已有 sections，不再渲染独立外部背景章节。
...
2. 产物验证完成：
   ...
   - 新 HTML 正文包含 `大盘观察`，并在其中清晰分出 `大盘定性` 和 `大盘结构`。
...
- 追加修正：每日复盘正文使用 `大盘观察`，并固定包含 `大盘定性` 与 `大盘结构` 两个子项；LLM sections schema、prompt、pytest 和 Promptfoo golden set 均已同步该要求。
