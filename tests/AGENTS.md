...
- `test_daily_review.py`：覆盖每日复盘研究的 `review-context.json`、Pydantic 校验、LLM sections 校验、HTML 首屏边界、partial 降级、missing 日期阻断、交易建议拒绝和公开 CLI 刷新契约。
- `test_cli.py`：覆盖仓库级 CLI 的 `daily-update` 和 `daily-review` 子命令、参数化日期、render mode、LLM output 参数和调用函数的参数传递。
