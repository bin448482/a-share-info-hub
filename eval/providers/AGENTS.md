...
- `run-a-share-daily-review.js`：Promptfoo 本地 provider，创建隔离 daily run 和可选 external background fixture，调用 `python -m a_share_info_hub daily-review --user-prompt ... --render-mode deterministic`，并返回 HTML 正文边界、external background 融合、唯一风险/问题板块和技术参考 Markdown 诊断审计结果。
